...
| 스킬 | 성격 | 용도 |
|------|------|------|
| `ib-generate` | **오케스트레이터** | 기획→도메인→와이어프레임→이슈를 한 번에 순차 생성 (단계 사이 체크포인트) |
| `ib-plan` | 생성 1단계 | 아이디어 → 기획서 적재 (명확화 질문 5~10개 필수) |
| `ib-domain` | 생성 2단계 | 도메인/컬럼 → 표·ERD·상태흐름도로 적재 (첫 설계는 초안) |
| `ib-wireframe` | 생성 3단계 | 기획 → 화면별 와이어프레임 적재 (IA 순서) |
| `ib-issues` | 생성 4단계 | 기획 → 이슈 트리 적재 (제목 `[영역]`, 완료 조건 `- [ ]`, 기획/화면/도메인 연동) |
| `ib-design` | 별도 | 메인 색 → 디자인 토큰 시스템 생성 |
| `ib-sync` | 유지보수 | 기획 개정 후 도메인·화면·이슈가 어긋난 곳을 찾아 보고 (반영은 사용자 선택) |
...
| `plan-spec.md` | 기획서 9섹션 마크다운 템플릿 + 언어 규칙(유비쿼터스 언어) |
...
/ib-generate 팀 협업용 칸반 앱을 만들고 싶어
   → (앱/접두사 확인) → 기획 → [확인] → 도메인 → [확인] → 와이어프레임 → [확인]
   → 이슈 → [확인] → 기획 확정(착수 게이트)
```
