...
기획의 유스케이스·핵심 기능을 커버하는 **주요 화면 목록**을 뽑는다. MVP에 필요한 만큼만.

그다음 **IA(Information Architecture) 관점에서 화면의 순서를 정한다.** 나열 순서가
아니라 사용자가 제품을 이해·이동하는 논리적 흐름을 반영해야 한다. 정렬 기준(위→아래):

1. **진입/인증** (예: 스플래시, 로그인, 회원가입)
2. **핵심 허브/홈** (로그인 후 최초로 도착하는 화면, 주 내비게이션 기준점)
3. **주요 태스크 플로우** (제품의 핵심 유스케이스를 사용자가 밟는 순서대로. 예:
   방 생성 → 대기실 → 레이스 → 결과)
4. **상세/보조 화면** (목록에서 파고드는 상세, 프로필 등)
5. **설정/기타** (마지막)

- 같은 그룹 안에서는 **사용자가 실제로 거치는 순서**로 둔다.
- IA 순서대로 **`sequence` 값을 0부터 연속**으로 매긴다(0, 1, 2, …). 이 값이 각
  와이어프레임의 **정렬 순서**가 되어(4단계), 대시보드 왼쪽 네비가 오름차순으로 정렬해
  IA 흐름대로 보여준다. **번호는 이름에 넣지 않는다** — 이름은 "로그인"처럼 순수하게 두고
  순서는 `sequence`가 담당한다.
- 각 화면에 **안정적 kebab-case id**(예: `login`, `race-detail`)도 부여한다
  (`data-screen`용, sequence와 무관하게 고정).

이 **sequence + 화면명 + id** 매핑을 사용자에게 먼저 보여주고 진행한다.
- 예: `0` 로그인(`login`) · `1` 홈(`home`) · `2` 방 생성(`room-create`) ·
  `3` 대기실(`room-lobby`) · `4` 레이스(`race`) · `5` 결과(`race-result`)
...
화면마다 한 번씩:
`create_wireframe(projectId, name="<화면명>", format="html", content=<HTML>, sequence=<IA순서>)`

- `name`은 화면별로 사람이 읽는 이름(예: "로그인", "레이스 상세"). **번호를 붙이지 마라.**
  재생성 시에도 **동일하게** 유지하라 — 같은 이름이라야 그 화면의 버전으로 누적된다.
- `sequence`는 2단계에서 정한 IA 순서(0부터). 재생성 시 **같은 값을 유지**하라 —
  생략하면 같은 이름의 기존 순서를 자동으로 잇는다(순서가 어긋나지 않음). IA를 재배치할
  때만 sequence를 새로 넘겨 순서를 조정한다.
