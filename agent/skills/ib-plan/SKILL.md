...
아이디어에서 **가정이 갈리는 지점**을 `AskUserQuestion`으로 묻는다. **최소 5개, 최대 10개** —
`AskUserQuestion`은 한 번에 4개까지만 담기므로 **여러 번 나눠 호출**한다(예: 4 + 4 + 2).
정말 갈리는 것만 묻되, 답에 따라 기획이 달라지는 축은 빠뜨리지 마라. 사용자가 이미 답한
축은 다시 묻지 않고, 스스로 세운 가정은 기획서 "핵심 가정/미결"에 명시한다.
물어볼 후보 축(해당될 때만):

- **스코프 / MVP 경계** — 이번 버전에 무엇까지 넣고 무엇을 뺄 것인가
- **타깃 플랫폼** — 웹 / 모바일 / 데스크톱 / CLI 중 무엇인가
- **핵심 제약** — 기술 스택, 일정, 규제, 기존 시스템 연동 등
- **사용자·권한 구조** — 역할 구분, 조직 규모, 권한 경계
- **핵심 도메인 규칙** — 정책·계산 방식·승인 흐름 등 도메인마다 갈리는 지점
- **외부 연동** — 인증(SSO), 결제, 메신저·알림, 기존 시스템 데이터 이관
- **운영·리포팅** — 감사 로그, 통계/대시보드, 데이터 내보내기 요구

### 2) 기획서 작성

**[ib-shared/plan-spec.md](../ib-shared/plan-spec.md)의 9섹션 템플릿·표현 규칙을 그대로 따른다.**
...
3. 기획 적재 — **신규인지 개정인지 구분**:
   ...
   - **기존 기획 개정**: `create_plan`으로 새로 만들지 말고 `update_plan(planId, content=<새 전체>)`로
     **작업본을 덮어쓴다**(초안 편집은 버전을 쌓지 않는다).
     - **변경은 본문 끝에 덧붙이지 말고 해당 섹션(1~9) 안에 녹인다.** 먼저 `get_plan`으로 현재
       본문을 읽고, 바뀐 내용을 알맞은 섹션·표·콜아웃에 반영해 **9섹션 템플릿을 유지한 채 전체를
